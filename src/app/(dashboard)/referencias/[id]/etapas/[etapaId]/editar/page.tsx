import { atualizarEtapa } from "@/app/actions";
...
export default async function EditarEtapaPage({
  ...
  const etapaIdInt = parseInt(etapaId);
  
  const [referencia, etapa] = await Promise.all([
    Prisma.referencia.findUnique({
      where: { id: refId },
      select: { codigo: true, nome: true }
    }),
    Prisma.etapaProducao.findUnique({
      where: { id: etapaIdInt }
    })
  ]);

  if (!referencia || !etapa) notFound();

  const updateAction = atualizarEtapa.bind(null, etapaIdInt);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Editar Etapa</h2>
      </div>

      <div className="bg-white p-4 rounded-lg border border-gray-100 mb-6">
        <p className="text-sm text-gray-500">Editando etapa de:</p>
        <p className="font-medium text-gray-900">{referencia.codigo} - {referencia.nome}</p>
      </div>

      <form action={updateAction} className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm space-y-6">
        <input type="hidden" name="referencia_id" value={refId} />
        
        {/* Note: Name is typically allowed to edit? Yes. */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1.5">
            Nome da Etapa
          </label>
          {/* Assuming we can edit name as well, though 'atualizarEtapa' in actions.ts logic check needed? */}
          {/* Checking previous read of actions.ts: atualizarEtapa logic: 
              status: ..., data_inicio: ..., data_fim: ..., observacoes: ...
              It DOES NOT update 'nome'. It seems 'nome' is fixed once created or I missed it.
          */}
          <input
            name="nome"
            required
        ...
        <div className="grid grid-cols-2 gap-6">
          <div>
            ...
            <input
              type="date"
              name="data_inicio"
              defaultValue={etapa.data_inicio ? etapa.data_inicio.toISOString().split('T')[0] : ''}
              className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-400"
            />
          </div>
          <div>
            ...
            <input
              type="date"
              name="data_fim"
              defaultValue={etapa.data_fim ? etapa.data_fim.toISOString().split('T')[0] : ''}
              className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-400"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <select
            name="status"
            defaultValue={etapa.status || "pendente"}
            className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-pink-500/20 focus:border-pink-400"
          >
